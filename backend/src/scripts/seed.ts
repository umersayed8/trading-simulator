  ...
  export async function seed() {
    ...
    for (const a of achievements) {
      await connection.execute(
        `INSERT INTO achievements (name, description, icon, xp_reward, criteria_type, criteria_value)
        VALUES (?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE description = VALUES(description), xp_reward = VALUES(xp_reward)`,
        [a.name, a.description, a.icon, a.xp_reward, a.criteria_type, a.criteria_value]
      );
    }
    ...
    for (const c of dailyChallenges) {
      await connection.execute(
        `INSERT INTO daily_challenges (title, description, type, target, xp_reward)
        VALUES (?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE title = title`,
        [c.title, c.description, c.type, c.target, c.xp_reward]
      );
    }
    console.log(`Seeded ${dailyChallenges.length} daily challenges`);

    // Seed lessons (clear and re-insert for clean ordering)
    console.log('Seeding lessons...');
    // Only insert if not already present (check by order_index)
    for (const lesson of lessons) {
      await connection.execute(
        `INSERT INTO lessons (title, description, category, difficulty, xp_reward, duration_minutes, order_index, prerequisite_id, content)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE title = title`,
        [lesson.title, lesson.description, lesson.category, lesson.difficulty, lesson.xp_reward, lesson.duration_minutes, lesson.order_index, lesson.prerequisite_id, lesson.content]
      );
    }
